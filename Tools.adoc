...
** On Windows: install the JDK (which itself comes with a private JRE). Do not install the public JRE (uncheck the box). Set the environment variables: JAVA_HOME to where you installed the JDK and https://docs.oracle.com/javase/9/install/installation-jdk-and-jre-microsoft-windows-platforms.htm#JSJIG-GUID-96EB3876-8C7A-4A25-9F3A-A2983FEC016A[PATH] to "%JAVA_HOME%\bin". Consult https://java.com/en/download/help/manual_regedit.xml[this] for help removing traces of previous install (if necessary). 
