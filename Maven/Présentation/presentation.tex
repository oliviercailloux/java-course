...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Exemple : \texttt{mvn compiler:compile}
		\item Appelle goal \texttt{compile} dans plugin \texttt{compiler}
		\item Compile sources dans répertoire destination
		\item \texttt{compiler} est le nom \emph{court} du plugin
		\item Coordonnées complètes : groupId, artifactId, version
		\item Version par défaut dépend de la version de Maven installée
		\item Commande complète : \texttt{mvn org.apache.maven.plugins: maven-compiler-plugin:3.8.0:compile} (en un seul mot)
	\end{itemize}
\end{frame}
