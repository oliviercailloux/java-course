...
* Go to the project properties and `Targeted Runtime`, select `My GlassFish 5 RE`.

You should now be able to deploy your project to your application server manually.
From Eclipse, export your project as an archive. Note that Eclipse is clever enough not to include the Runtime Environments libraries in your archive. (You can see that by uncompressing the produced archive.) Put the archive inside the relevant folder in your application server (e.g. `/usr/local/share/glassfish5/glassfish/domains/domain1/autodeploy`). Check that your application is reachable.
...
* Use the context menus: `New` (name your server `My GlassFish 5 server`), then use `Add and Remove`, `Publish`, and other options as needed.
