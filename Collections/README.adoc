...
// set (long)
* https://math.hws.edu/javanotes/c10/exercises.html[EE10.2] [most important]
// read, sort a list
...
* Write a static method that takes a `String` (supposedly, a sentence) as parameter, and returns a `Map<String, Integer>` that associates each word in the sentence to the number of times it appears in the sentence. For example, if the input is “the cat and the hat”, the output is a map associating “the” to 2, “cat” to 1, “and” to 1, and “hat” to 1. You can suppose that words are separated by spaces, and that there is no punctuation. Use Java maps and interfaces appropriately.
* Transform your static method into an instance method with no parameter. The String parameter is now a member of the instance.
* Using an `ImmutableMap` (see the Guava doc), invert the map. Why do you now get collections as values? Use this technique to create methods that return the word seen only once (if there is a single such word), the word seen the most, and the set of words that are repeated.
