...
A client might send an HTTP request, targeted to http://example.com/mycat, specifying that the chosen HTTP method is HTTP GET. The server then replies with an HTTP response with a header “content-type = image/jpeg” and a message body containing an image of a cat.
...
.An example of a Java EE servlet [source, java]
--
include::README.adoc[]
--
