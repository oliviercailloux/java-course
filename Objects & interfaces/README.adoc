...
See: https://github.com/oliviercailloux/java-course/raw/master/Objects%20%26%20interfaces/Pr%C3%A9sentation/presentation.pdf[présentation]; or read https://math.hws.edu/javanotes/c5/[ES5] intro to 5.4.

== Instance variables and methods: syntax
See https://github.com/oliviercailloux/java-course/blob/master/Syntax/Classes%20%as%concept.adoc[Classes as concept] for the syntax about instance variables and instance methods
...
== Inheritance
See: https://github.com/oliviercailloux/java-course/blob/master/Syntax/Inheritance.adoc[Inheritance]
...
== Exercice: Checked exceptions
Program a method that draws a random number and throws a `IOException` with one chance on two. Observe that it compiles only if you declare this exception in the signature. Call this method from another method. Observe that the calling method also must declare the exception. Change the exception to an `IllegalArgumentException`. Observe that you do not have to declare this in either method signature.
...
* Example using Java interface: see this https://raw.githubusercontent.com/oliviercailloux/Simple-Papyrus-project/interface/An%20interface%20and%20two%20realizations.svg[example diagram] and the corresponding https://github.com/oliviercailloux/sample-interface[project] (to be imported in Eclipse)
