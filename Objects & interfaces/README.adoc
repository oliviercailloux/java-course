...
== Two players
* Implement a two players (random) game. Each player has a triplet of dice. Each player throw their dice. Player one loses one point and rethrows if the other player has at least as high a total as hers. Then same thing for player two. After twenty turns, the highest total wins.
Example run: P1 has 10, P2 has 7. P1 keeps her roll. P2 loses one point and rerolls and obtains 14. P1 loses one point and rerolls and obtains 11. P2 keeps her roll.
* Create a class Player that contains her dice and her current result. Implement the game by using this class.
