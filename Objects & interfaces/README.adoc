...
== Multiple players
* Implement a two players (random) game. Each player has a triplet of dice. Each player rolls their dice. Player one loses one point and rerolls if the other player has at least as high a total as hers. Then same thing for player two. After twenty turns, the highest total wins.
