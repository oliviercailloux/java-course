...
* Ne réinventez pas la roue ! Cela augmente votre base de code, donc rend la maintenance plus difficile. Réutilisez les bibliothèques existantes. Si le problème que vous souhaitez résoudre est raisonnablement commun, il existe sûrement une bibliothèque de bonne qualité prête à être réutilisée. (Si vous ne tenez pas compte de ce conseil, je considérerai votre code écrit inutilement comme pénalisant pour votre note s’il contient des imperfections ; et jamais de façon positive.)
...
* (Lorsque les collections ont été vues en cours.) Privilégier des collections (List, Set, etc.) plutôt que des tableaux de types primitifs. Cela simplifie généralement l’extension future, et clarifie.
* (Lorsque les tests unitaires ont été vus en cours.) Vos fonctionnalités doivent être couvertes par des tests unitaires, considérés comme partie intégrante de chaque tâche et soumis en même temps que le code testé.
