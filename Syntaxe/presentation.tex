...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item \texttt{static <type de retour> <nom> (<type param1> <nom param1>, …)}
		\item \texttt{return} pour renvoyer valeur (exécution méthode cesse)
		\item \texttt{void} pour indiquer absence de retour
		\item Méthode statique peut être vue comme une fonction
	\end{itemize}
	\begin{block}{Exemple}
		\begin{lstlisting}
static double weightedSum(double a, double b, 
      double w1, double w2) {
	return w1 * a + w2 * b;
}
static void useWeightedSum() {
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \texttt{class MyClass} suivi d’un bloc
		\item Sert (entre autres) à grouper les méthodes
		\item Nom de la méthode préfixé du nom de la classe
	\end{itemize}
	\begin{block}{Exemple}
		\begin{lstlisting}
class MyMathClass {
