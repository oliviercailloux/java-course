...
** Material: http://math.hws.edu/javanotes/c1/[ES1.1], 1.2, 1.3, 1.4, 2 intro, 2.1, 2.2, 2.3.3, 2.4.1, 2.5, 2.6.6; ES3 intro to 3.6 [Should be known or easy]
** ES1.5
** ES1.6, 1.7 [O]
...
* https://github.com/oliviercailloux/java-course/raw/master/Exécution/presentation.pdf[Exécution]
* Exercices:
...
** Compile `MyBooleanClass`, move the resulting file into its own folder, execute it from the source folder.
** Compile `MyMathClass`, move the resulting file into its own folder (alone), and execute it from the source folder. Why does it fail? What does the error message say, and how is it related to the problem? Fix the problem and execute it, first by grouping the class files, second, while keeping both class files in different folders.
