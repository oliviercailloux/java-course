...
* Graded programming exercice (Maven; contract; fail fast; objects). Create a Maven project and push your whole project to the GitHub repository (so that the `pom.xml` is at the root of the GitHub repository). Make sure the POM follows the best practices as indicated in the course (hint: use the “Modern project” archetype). Follow https://github.com/oliviercailloux/java-course/blob/master/Best%20practices/Git.adoc[Git Best practices].
** Hint: Use https://github.com/oliviercailloux/java-course/blob/master/Best%20practices/Eclipse.adoc[correct] Eclipse config. (Check warnings, compile errors, instructions!)
...
** Provide methods `getLeft()`, `getRight()`, that each return the apporpriate type, and `setContents()`, that admits two parameters and that set both instances of the pair.
