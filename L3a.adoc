...
// * https://classroom.github.com/a/aiweSTSU[Git] (marked /10): use this repository (created when you accept the homework) at the last step of the git exercices above _instead of creating a new remote remository RG_. Connect your local repository R1 to it, and keep on following the instructions in the exercice. I do not expect written answers to the questions asked in the exercice. Your GitHub username and git `user.name` must be identical.

[[S3]]
=== S3 (Thursday 21st of March, 2019)

* Graded exercice similar to Dep-Git
