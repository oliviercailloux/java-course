...
* Graded test: score-keeper. About referring to objects from other objects. You will have 60 minutes. There will be several interfaces, you only have to implement `MyScoreManager`. (You may need to implement other classes to achieve this.) There is no documentation in this project: you will have to read the unit tests to understand what the class is supposed to do.
