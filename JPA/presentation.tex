...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \texttt{em.persist(person)}%TODO table requiert association des deux côtés, em.persist le fait en une fois ?
		%TODO parler de auto flush par défaut : flush before query (if overlap), en Hibernate.
		...
		\item NB cascade va toujours du parent (\texttt{person}) vers ses enfants (\texttt{phone}s)%TODO vérifier
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Ou effacement automatique avec \texttt{orphanRemoval} sur annotation \texttt{OneToMany}%TODO mieux vaudrait alors utiliser une collection de valuetypes !
