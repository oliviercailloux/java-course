...
* You should test non-trivial code: if the code is so simple that it cannot conceivably fail (such as one-line code as in a simple getter or `toString` method), do not test it
...
* Instead of Importing the class with `import `org.junit.jupiter.api.Assertions;` and using it with `Assertions.assertEquals(…)`, import the method with `static import `org.junit.jupiter.api.Assertions.assertEquals;` and use it with `assertEquals(…)`.
