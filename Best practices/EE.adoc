...
Be cautious however. This (unfortunately) includes directly https://www.javadoc.io/doc/javax/javaee-api/7.0/index.html[many classes] which also are available as separate packages. And sometimes those classes are older versions that those provided by recent packages. Hence, you might end up with different classes in your classpath that have the exact same name. And problems might follow.
