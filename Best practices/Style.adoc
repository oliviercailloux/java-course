...
Use javadoc style comments to comment your code (`/** This explains something. */`). Do not use comments to comment out code (`//object.someMethod()`). This hinders readability and increases the length of the code, and quickly becomes completely unuseful as commented code accumulate. You should anyway use a versioning control system and use this to go back in time when desired.
