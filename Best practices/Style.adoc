...
Do not use comments to comment out code (`//object.someMethod()`). This hinders readability and increases the length of the code, and quickly becomes completely unuseful as commented code accumulate. You should, instead, use a versioning control system and use this to go back in time when desired.
