...
== Devoirs
* Cliquez sur le lien indiqué sur le devoir approprié pour que GitHub classroom vous crée un dépôt privé où soumettre votre devoir.
...
* Le servlet utilise JUL pour logger les valeurs des paramètres envoyés en entrée et la valeur de sortie renvoyée.
* Le servlet répond aussi à un autre type de requête, destinée à indiquer la valeur par défaut pour le deuxième nombre (`param2`). À vous de choisir un type correct (la méthode HTTP) pour cette requête. Cette requête indique (en `text/plain`) la valeur par défaut pour le deuxième nombre si manquant. Le servlet répond `ok` en `text/plain` si la requête a été comprise. Après ceci, le servlet doit en tenir compte lors de sa réponse à la requête du premier type décrite ci-dessus.
