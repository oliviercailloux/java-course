...
** Une tâche est effectuée en une seule itération. Si votre tâche poursuit une tâche d’une livraison précédente, il s’agit quand-même d’une nouvelle tâche. Créez une nouvelle PR, pour que je m’y retrouve, et indiquez le binôme qui s’en occupe (qui devrait avoir au moins partiellement changé).
...
* [[separate]]Je dois voir vos changements facilement et les distinguer de code auto-généré ou importé sans changements de votre part. Si par exemple vous ajoutez un canevas HTML de 100 fichiers, faites-le dans une tâche dédiée, et créez une autre tâche pour vos modifications. (SEP)
...
| SEP	| Séparer les tâches | https://github.com/oliviercailloux/java-course/blob/master/Divers/Projets.adoc#separate[Projets#separate] | 
|===
